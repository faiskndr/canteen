     ...
     >
        <div class="w-full max-w-2xl mx-auto">
          <div class="bg-white border-4 border-gray-800 p-8">
            ...
            <p class="text-gray-600 mb-6">Isi data petugas</p>
            ...
            <form wire:submit="save" class="space-y-4">
              <div>
                ...
                <input
                  type="text"
                  wire:model="petugasForm.username"
              ...
              <div>
                ...
                <input
                  type="password"
                  wire:model="petugasForm.password"
                  class="w-full border-4 border-gray-800 p-3 focus:outline-none focus:ring-4 focus:ring-gray-400"
                  placeholder="••••••••"
                  required
                />
              ...
              <div>
                ...
                <select
                    wire:model="petugasForm.user_group_id"
                    wire:key="user-group-select-{{ $petugasForm?->user_group_id }}"
                    class="w-full border-4 border-gray-800 p-3 focus:outline-none focus:ring-4 focus:ring-gray-400"
                    required
                >
                 <option value="">-- Pilih Usergroup --</option>
                @foreach($userGroupList as $u)
                    <option value="{{ (string) $u->user_group_id }}">{{ $u->nama }}</option>
                @endforeach
                </select>
              </div>
              ...
              <div>
                ...
                <select
                    wire:model="petugasForm.kantin_id"
                    class="w-full border-4 border-gray-800 p-3 focus:outline-none focus:ring-4 focus:ring-gray-400"
                >
      ...
      <div class="w-full max-w-6xl mx-auto">
        ...
        <div class="space-y-4">
            @forelse($petugas as $p)      
              <div key={s.id} class="bg-white border-4 border-gray-800 p-6">
                <div class="flex items-center gap-4">
                  <div class="w-16 h-16 border-4 border-gray-800 flex items-center justify-center bg-gray-100 flex-shrink-0">
                    <UserCog size={32} />
                  </div>
                  <div class="flex-1">
                    <h3 class="font-bold text-xl">{{ $p->username }}</h3>
                    <p class="text-sm text-gray-600">Role: {{ $p->userGroupRelation->nama }}</p>
                    <p class="text-sm text-gray-400">Password: *******</p>
                  </div>
                  <div class="flex gap-2">
                    <button
                        wire:click="open({{ $p->user_id}} )"
                        class="border-4 border-gray-800 p-3 hover:bg-gray-100"
                    >
                    ...
                    <button
                      class="border-4 border-red-500 p-3 hover:bg-red-50"
                    >
            ...
            @empty
                <div class="bg-white border-4 border-gray-800 p-12 text-center">
                    <p class="text-gray-500 font-bold">PETUGAS TIDAK DITEMUKAN</p>
                </div>
            @endforelse
        </div>
      </div>
